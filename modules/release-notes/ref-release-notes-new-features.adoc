...
[id="feature-rhdhplan-1089"]
= Images can now be fetched without authentication

To simplify the process for pulling and inspecting {product-very-short} images, you can now access them from the unauthenticated {rhec}. Use `registry.access.redhat.com` instead of `registry.redhat.io` when pulling images.

// Link in comment to facilitate reviews:https://redhat.atlassian.net/browse/RHDHPLAN-1089
