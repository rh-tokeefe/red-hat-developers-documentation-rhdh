...
[id="bug-fix-rhdhbugs-2825"]
=== Helm chart and Operator compatible with 1.8.9 Orchestrator plugin versions

In {product-short} 1.8.5, the Helm chart and Operator have been updated to use the 1.8.9 Orchestrator plugin versions from the Red Hat NPM registry. This ensures that previously fixed Orchestrator-related bugs are included in the supported version.

.Additional resources
* link:https://redhat.atlassian.net/browse/RHDHBUGS-2825[RHDHBUGS-2825]

[id="bug-fix-rhdhbugs-2995"]
=== Helm chart and Operator compatible with 1.8.12 Orchestrator plugin versions

Previously, {product-short} failed to establish a connection with the the orchestrator backend plugin and {product-short} generated a `Request failed with status 413` error. Now, you can configure the content length limit in the `app-config.yaml` file.

Uncomment to set the content length limit for the requests. The default setting is `102400` bytes (100kb).
...
.Additional resources
* link:https://redhat.atlassian.net/browse/RHDHBUGS-2995[RHDHBUGS-2995]